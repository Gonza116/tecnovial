...
function consultaPracticasPorHacerAlumno($conexion, $id){
    try{
        $stmt = $conexion->prepare("SELECT * FROM CLASE_PRACTICA WHERE (NUM_ALUM = :id AND VALORACION IS NULL) ORDER BY FECHA");
        $stmt->bindParam(':id',$id);
        $stmt->execute();
...
function consultaPracticasAlumno($conexion, $id){
    try{
        $stmt = $conexion->prepare("SELECT * FROM CLASE_PRACTICA WHERE (NUM_ALUM = :id AND VALORACION IS NOT NULL) ORDER BY FECHA");
        $stmt->bindParam(':id',$id);
        $stmt->execute();
        return $stmt->fetchAll();
     } catch(PDOException $e) {
...
function consultarClasesTeoricas($conexion, $num_alum){
    try{
        $stmt = $conexion->prepare("SELECT * FROM TEST_TEORICO WHERE NUM_ALUM = :id ORDER BY FECHA");
...
function consultarExamenesTeoricosNoHechos($conexion, $id){
    try{
        $stmt = $conexion->prepare("SELECT * FROM EXAMEN_TEORICO_DGT WHERE (NUM_ALUM = :id AND APTO IS NULL) ORDER BY FECHA_EX");
        $stmt->bindParam(':id',$id);
        $stmt->execute();
        return $stmt->fetchAll();
     } catch(PDOException $e) {
...
function consultarExamenesPracticosHechos($conexion, $id){
    try{
        $stmt = $conexion->prepare("SELECT * FROM EXAMEN_PRACTICO_DGT WHERE (NUM_ALUM = :id AND APTO IS NOT NULL) ORDER BY FECHA_EX");
        $stmt->bindParam(':id',$id);
        $stmt->execute();
        return $stmt->fetchAll();
     } catch(PDOException $e) {
...
function consultarExamenesPracticosNoHechos($conexion, $id){
    try{
        $stmt = $conexion->prepare("SELECT * FROM EXAMEN_PRACTICO_DGT WHERE (NUM_ALUM = :id AND APTO IS NULL) ORDER BY FECHA_EX");
        $stmt->bindParam(':id',$id);
        $stmt->execute();
        return $stmt->fetchAll();
     } catch(PDOException $e) {
...
function consultarTodosExamenesPracticos($conexion){
    try{
        $stmt = $conexion->prepare("SELECT * FROM EXAMEN_PRACTICO_DGT NATURAL JOIN ALUMNOS WHERE APTO IS NULL ORDER BY FECHA_EX");
        $stmt->execute();
        return $stmt->fetchAll();
     } catch(PDOException $e) {
		$_SESSION['excepcion'] = $e->getMessage();
...
function consultarTodosExamenesTeoricos($conexion){
    try{
        $stmt = $conexion->prepare("SELECT * FROM EXAMEN_TEORICO_DGT NATURAL JOIN ALUMNOS WHERE APTO IS NULL ORDER BY FECHA_EX");
        $stmt->execute();
        return $stmt->fetchAll();
     } catch(PDOException $e) {
		$_SESSION['excepcion'] = $e->getMessage();
...
function consultarTodasClasesPracticas($conexion, $id_prof){
    try{
        $stmt = $conexion->prepare("SELECT * FROM CLASE_PRACTICA NATURAL JOIN ALUMNOS NATURAL JOIN COCHES WHERE (VALORACION IS NULL AND NUM_PROF = :id_prof) ORDER BY FECHA");
        $stmt->bindParam(':id_prof', $id_prof);
        $stmt->execute();
        return $stmt->fetchAll();
     } catch(PDOException $e) {
...
function consultarDNIs($conexion) {
    try{
        $consulta = "SELECT DNI FROM ALUMNOS";
        return $conexion->query($consulta);
    } catch(PDOException $e) {
		$_SESSION['excepcion'] = $e->getMessage();
        header("Location: excepcion.php");
